...
as modeling approach and then further split into a training and testing set for 
model optimization.
...
The `rating` column represents the outcome to be predicted.  
...
```{r data_exploration_ratings, echo=F, fig.height=3}
...
```{r data_exploration_sparsity, fig.height=7, echo=F}
...
```{r data_exploration_ratings_movie, echo=F, fig.height=3}
...
```{r data_exploration_ratings_user, echo=F, fig.height=3}
...
A wide range in number of ratings per movie and per user is present with some
users having rated up to `r max(pull(ustat, prop) * 100) %>% round(digits = 2)` 
% of all available movies and some movies having been rated by up to
`r max(pull(mstat, prop) * 100) %>% round(digits = 2)` % of all available users.
For other movies, in contrast, only `r min(pull(mstat, n))` rating is present
and for some users only `r min(pull(ustat, n))` ratings are available.  
...
On a per-user level the relation between the average rating and the number of
ratings per user is not as strong, even though users with a very high number of
rated movies tend to have a below-average mean rating.
...
```{r data_exploration_genres_main, echo=F, fig.height=3}
...
```{r data_exploration_genres_comb, echo=F, fig.height=3}
...
```{r data_exploration_time_rating, fig.height=3, echo=F}
...
```{r data_exploration_time_publication, fig.height=4, echo=F}
...
especially since mean ratings for years with very few data points cannot be 
fully considered.
The effect by rating weekday seems to be even smaller, with the mean of average
ratings per movie ranging from
`r group_by(rday.df, rating_weekday) %>% summarize(avg = mean(avg_rating)) %>% 
pull(avg) %>% min() %>% round(digits = 2)` to
`r group_by(rday.df, rating_weekday) %>% summarize(avg = mean(avg_rating)) %>% 
pull(avg) %>% max() %>% round(digits = 2)` across all weekdays.
The effect of publication year on average movie ratings in terms of rating means
per publication year ranges from
`r group_by(pubyear.df, year) %>% summarize(avg = mean(avg_rating)) %>% 
pull(avg) %>% min() %>% round(digits = 2)` to
`r group_by(pubyear.df, year) %>% summarize(avg = mean(avg_rating)) %>% 
pull(avg) %>% max() %>% round(digits = 2)`, although the estimated average 
rating for the first publication years is again rather noisy due to the limited 
number of movies published in these years.  
...
Further, the specified combinations of movie genres seem to provide additional 
information on movie ratings and are thus included in the model as well.
Rating year, rating weekday and publication year, however, showed a rather
small impact on ratings and are therefore not included in the model.  

The resulting model for movie ratings $Y$ with the overall average rating $\mu$ 
as baseline and level-specific feature effects $b$ for movie $m$, user $u$ and 
movie genre $g$ as well as an error term $\epsilon$ can be written as follows:  
...
Since for some movies, users and genres only few ratings are available, 
regularization of coefficients is additionally used to improve model 
performance.
Here, the following function will be minimized to obtain the best value for 
the penalty parameter $\lambda$:

$$\frac{1}{N}\sum{(Y - \mu - b_m - b_u - b_g)^2} + \lambda (\sum{b_m^2} + 
\sum{b_u^2} + \sum{b_g^2})$$
...
optimal value for $\lambda$ that minimizes the RMSE on the testing set is 
selected.
For computational efficiency, the same value for $\lambda$ is used across all
...
```{r lambda, echo=F, fig.height=2.5}
...
as naive prediction is also calculated.    

```{r modeling_evaluation}
...
```{r modeling_evaluation_kable, echo=F}
...
average, the simple additive model, in which movie effects, user effects and 
genre effects are considered, yields a considerably lower RMSE of 
`r round(rmse_basic, digits = 5)`.
...
[HarvardX's Data Science Professional Certificate](https://www.edx.org/professional-certificate/harvardx-data-science),
the selection of the modeling approach was to some extend based on the lessons
learned in the preceding
[HarvardX's Data Science Machine Learning](https://www.edx.org/course/data-science-machine-learning) 
course.
Further improvements in model performance may be achieved by employing
